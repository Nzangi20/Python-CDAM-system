...
def seed_sessions() -> None:
    ...
    if not version_setting:
        version_setting = PlatformSetting(key="curriculum_version", value="8")
        db.session.add(version_setting)
        db.session.commit()
        force_update = True
    elif version_setting.value != "8":
        version_setting.value = "8"
        db.session.commit()
        force_update = True
        ...
        try:
            ...
            if "sessions" in inspector.get_table_names():
                ...
                if (r_db_count == len(R_SESSIONS) and 
                    ...
                    version_setting and version_setting.value == "8"):
